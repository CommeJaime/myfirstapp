...
function AnimeCard({ cover, name, id, episodes }) {
	...
	return (
		<a className='item' href={redirect}>
			<div style={stylesImg} onMouseEnter={() => setOpacity(0.5)} onMouseLeave={() => setOpacity(1)}>
			...
			<img onLoad={() => setOpacity(1)}
				 ...
				 className="item-play" src={play}  alt="" />
			<p style={stylesName} onMouseEnter={() => setColor("blue")} onMouseLeave={() => setColor("")} className="item-name">{name}</p>
		</a>
	)
}
